...
#fix wall1 anchor wall/lj126 10.0 ${sigma_polymer} ${cutoff_polymer} zlow EDGE # Attractive to the wall 
#fix wallzlo forcegroup wall/lj126 zlo EDGE 10.0 ${sigma_polymer} ${cutoff_polymer}
#fix wallzhi forcegroup wall/lj126 zhi EDGE 10.0 ${sigma_polymer} ${cutoff_polymer}
...
dump 1 all custom 10 1_soft.lammpstrj id  type  x y z  ix iy iz
thermo          1
timestep        0.002
run 2000
undump 1
unfix 4
dump 1 all custom 1000 2_hard.lammpstrj id  type  x y z  ix iy iz
...
dump 1 all custom  100  3_equil.lammpstrj  id  type  x y z  ix iy iz
fix 2 all nve
fix 3 forcegroup langevin 1.0 1.0 1 ${seed} zero yes
fix 5 forcegroup momentum 1 linear 1 1 1 angular rescale
thermo          1000
...
fix             s1 moving srd 5 NULL 1.0 1.0 49894 rescale no tstat yes shift yes 1234 beadgroup forcegroup  fflow 0.0001 0.0001 rcyl ${inradius}  virtual 10 alpha 120
#fix  wallsrd solvent wall/srd zlo EDGE zhi EDGE
#control variables
...
reset_timestep 0
compute 		cc1 solvent chunk/atom bin/3d x 0.0 0.5 y 0.0 0.2 z 0.0 0.2 units box
...
dump 1 output custom  1000  4_flow.lammpstrj  id  type  x y z  ix iy iz
run		1000000
